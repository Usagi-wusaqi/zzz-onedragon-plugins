...
**构造参数**:
- `app_const`: 常量模块，必须定义 `APP_ID`、`APP_NAME`、`DEFAULT_GROUP`、`NEED_NOTIFY`

`ApplicationFactory` 会从 const 模块读取应用 ID、显示名称、默认组和通知配置。旧版 `app_id=...` / `app_name=...` / `default_group=...` / `need_notify=...` 关键字参数写法已经不兼容当前主程序。
...
- 必须在构造函数中传入 const 模块，由 `ApplicationFactory` 统一读取 `APP_ID`、`APP_NAME`、`DEFAULT_GROUP`、`NEED_NOTIFY`
...
3. **const 必须字段**: 必须定义 `APP_ID`, `APP_NAME`, `DEFAULT_GROUP`, `NEED_NOTIFY`
...
8. **工厂初始化**: 当前版本传入整个 const 模块，如 `ApplicationFactory.__init__(self, my_plugin_const)`
9. **第三方插件**: 第三方插件目录被 gitignore，用户需要自行备份
10. **插件元数据**: 建议填写 `PLUGIN_AUTHOR`、`PLUGIN_VERSION` 等元数据以便用户识别
11. **相对导入**: 第三方插件完整支持相对导入，建议添加 `__init__.py` 文件
12. **导入主程序**: 第三方插件可以直接 `from one_dragon.xxx` 或 `from zzz_od.xxx` 导入主程序模块
13. **同目录冲突**: 同一目录下不允许多个 `_factory.py` 或 `_const.py` 文件，发现时整个目录被跳过
